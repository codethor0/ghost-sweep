...
**Historical document (archived Batch 10B).** Public launch completed in Batches 8A–9C. Baseline tables below reflect pre-launch state at document authoring time, not the current live status. See [implementation-status.md](../implementation-status.md) and [post-launch-roadmap.md](../post-launch-roadmap.md).
