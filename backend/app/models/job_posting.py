...
class JobPosting(Base, UUIDPrimaryKeyMixin):
    ...
    source: Mapped[PostingSource] = mapped_column(default=PostingSource.OTHER, index=True)
    posted_date: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    status: Mapped[PostingStatus] = mapped_column(default=PostingStatus.ACTIVE, index=True)
